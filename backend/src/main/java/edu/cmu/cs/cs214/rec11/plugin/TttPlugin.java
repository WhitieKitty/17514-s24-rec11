package edu.cmu.cs.cs214.rec11.plugin;

import edu.cmu.cs.cs214.rec11.games.TicTacToe;
import edu.cmu.cs.cs214.rec11.framework.core.GameFramework;
import edu.cmu.cs.cs214.rec11.framework.core.GamePlugin;

public class TttPlugin implements GamePlugin<TicTacToe.Player> {
    private static final String GAME_NAME = "Tic Tac Toe";
    private static final int GRID_WIDTH = TicTacToe.SIZE;
    private static final int GRID_HEIGHT = TicTacToe.SIZE;

    private TicTacToe ticTacToe;
    private GameFramework framework;

    @Override
    public String getGameName() {
        return GAME_NAME;
    }

    @Override
    public int getGridWidth() {
        return GRID_WIDTH;
    }

    @Override
    public int getGridHeight() {
        return GRID_HEIGHT;
    }

    @Override
    public void onRegister(GameFramework f) {
        framework = f;
    }

    @Override
    public void onNewGame() {
        ticTacToe = new TicTacToe();
        for (int i = 0; i < GRID_HEIGHT; i++) {
            for (int j = 0; j < GRID_WIDTH; j++) {
                framework.setSquare(i, j, "");
            }
        }
        framework.setFooterText(GAME_NAME + ": Player " + ticTacToe.currentPlayer().name() + "'s turn");
    }

    @Override
    public void onNewMove() {
        // No special setup needed before each move
    }

    @Override
    public boolean isMoveValid(int x, int y) {
        return ticTacToe.isValidPlay(x, y);
    }

    @Override
    public boolean isMoveOver() {
        // No special condition for move to be over
        return false;
    }

    @Override
    public void onMovePlayed(int x, int y) {
        String currentPlayer = ticTacToe.currentPlayer().name();
        ticTacToe.play(x, y);
        framework.setSquare(x, y, currentPlayer);
        if (ticTacToe.isOver()) {
            framework.setFooterText(GAME_NAME + ": Game over");
        } else {
            framework.setFooterText(GAME_NAME + ": Player " + ticTacToe.currentPlayer().name() + "'s turn");
        }
    }

    @Override
    public boolean isGameOver() {
        return ticTacToe.isOver();
    }

    @Override
    public String getGameOverMessage() {
        TicTacToe.Player winner = ticTacToe.winner();
        if (winner != null) {
            return "Player " + winner.name() + " wins!";
        } else {
            return "It's a draw!";
        }
    }

    @Override
    public void onGameClosed() {
        // No special cleanup needed
    }

    @Override
    public TicTacToe.Player currentPlayer() {
        return ticTacToe.currentPlayer();
    }
}
